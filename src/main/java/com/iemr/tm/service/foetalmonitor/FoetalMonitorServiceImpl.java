...
import com.iemr.tm.data.foetalmonitor.FoetalMonitorData;
...
import com.iemr.tm.utils.http.HttpUtils;
...
public class FoetalMonitorServiceImpl implements FoetalMonitorService {
	...
	/***
	 * @author DU20091017 update the FoetalMonitor data in the DB
	 * @throws IOException
...
//		FileInputStream file = new FileInputStream(filePath);
	...
	public String sendFoetalMonitorTestDetails(FoetalMonitor request, String auth) throws Exception {
		...
		try {
			...
			// Saving FoetalMonitor Data in Amrit DB
			...
			if (request != null && request.getFoetalMonitorID() > 0) {
				...
				if (deviceIDForVanID != null && deviceIDForVanID.getDeviceID() != null) {
					...
					throw new RuntimeException("Spoke is not mapped with foetalMonitor deviceID");
